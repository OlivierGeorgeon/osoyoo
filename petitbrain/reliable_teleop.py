...
# This code is used to tele-operate the robot.
...
# The PC keeps resending string until it receives the outcome string.
...
class ReliableTeleop:
    def __init__(self, ip, time_out, port=8888):
        self.IP = ip
        self.port = port
        self.socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.socket.settimeout(0)  # The socket's time out is 0. Not to be confused with the timeout used by code
    ...
    def main(self):
        """Executed every 100ms in the main loop"""
        ...
        # If the robot is enacting an interaction
            ...
            # Timeout: reinitialize the cycle. This will resend the command string
            else:
...
# py reliable_teleop.py 192.168.8.242
...
if __name__ == '__main__':
    ...
    reliable_teleop = ReliableTeleop(robot_ip, UDP_TIMEOUT)
    keyboard.on_press(reliable_teleop.generate_command_string)

    while running:
        reliable_teleop.main()
        time.sleep(0.1)
