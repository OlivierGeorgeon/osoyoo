    ...
    def select_enaction(self, enaction):
        ...
        # If there is an object to arrange
            ...
            if enaction.outcome_code == OUTCOME_FLOOR:
                self.step = STEP_INIT
                enaction_memory = self.workspace.memory.save()
                enaction_memory.egocentric_memory.prompt_point = np.array([-500, 0, 0])
            ...
            # If object behind target just watch (minus the radius to prevent keeping pushing)
                ...
                enaction_memory = self.workspace.memory.save()
                enaction_memory.egocentric_memory.prompt_point = None
                enaction_memory.egocentric_memory.focus_point = object_center  # Look at object
                composite_enaction = Enaction(self.workspace.actions[ACTION_WATCH], enaction_memory)
                self.step = STEP_INIT
