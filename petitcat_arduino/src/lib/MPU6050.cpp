...
bool MPU6050::begin(mpu6050_dps_t scale, mpu6050_range_t range, int mpua)
{
	...
	Wire.setWireTimeout( 25000, true);   // 1000 µs is 1 ms timeout
	...
	// Serial.println("Check MPU6050 Who Am I Register");
    // Serial.println(fastRegister8(MPU6050_REG_WHO_AM_I));
    fastRegister8(MPU6050_REG_WHO_AM_I);
    //if (fastRegister8(MPU6050_REG_WHO_AM_I) != 0x72) \\ 0x68)
    //{
	 // return false;
    //}
...
Vector MPU6050::readRawAccel(void)
{
    ...
    Wire.beginTransmission(mpuAddress);
    int wireresult = Wire.requestFrom(mpuAddress, 6);

    //while (Wire.available() < 6); // I tried to comment it to solve the imu freezing problem apparently caused by noise from motors
...
Vector MPU6050::readRawGyro(void)
{
    ...
    Wire.beginTransmission(mpuAddress);
    int wireresult = Wire.requestFrom(mpuAddress, 6); // OG

    // while (Wire.available() < 6); // OG
