...
class CtrlHexaview :
            ...
            def on_text_hemw(text):
                if model.need_user_action :
                    if text.upper() == "Y":
                        ...
                        model.need_user_action = False
                elif not model.synthesizer.synthetizing_step == 1 and not model.need_traitement_flag :
    ...
    def main_depr(self,dt):
        """Function that refresh the views"""
        model = self.model
        if not model.need_traitement_flag :
            ...
            if self.refresh_count >= 5000 : # We keep adding shapes, so every once in a while we must clean everything to avoid memory shortages
                ...
                self.hexaview.extract_and_convert_interactions(model.hexa_memory)
    ...
    def main(self,dt):
        ...
        if len(model.hexa_memory.recently_changed_cells) > 0 :
           self.hexaview.extract_and_convert_recently_changed_cells(model.hexa_memory)
           model.hexa_memory.cells_changed_recently = []
